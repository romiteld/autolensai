...
export function SocialShare({ vehicle }: SocialShareProps) {
  const [copied, setCopied] = useState(false);

  const shareUrl = typeof window !== 'undefined' ? window.location.href : '';
  const shareTitle = `${vehicle.year} ${vehicle.make} ${vehicle.model} - AutoLensAI`;
  const shareText = vehicle.description || 
  ...
  const shareLinks = {
    ...
    twitter: `https://twitter.com/intent/tweet?text=${encodeURIComponent(`${shareTitle} - ${shareText}`)}&url=${encodeURIComponent(shareUrl)}`,
  ...
  return (
    <Card className="p-6">
      <h2 className="text-lg font-semibold mb-4">Share This Vehicle</h2>
